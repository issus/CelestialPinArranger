...
namespace SymbolBuilder.Mappers
{
    public class DefaultMapper : PinMapper
    {
        ...
        public override void LoadMappings()
        {
            ...
            AddFunction(string.Empty, PinClass.PowerSupply,         "Ground",           "^GND(?:\\d+)?$|^VSS|^IOVSS|^PGND|^EP$|^EPAD$|^DGND|^VEE|^DVEE|^PVEE|^PAD$|^DRVSS|^VSS_RF(?:\\d+)?$|^GND", PinPosition.From(PinSide.Left, PinAlignment.Lower));
            ...
            AddFunction(string.Empty, PinClass.IOPort, "Port", "^(?:P(?<Group>\\w)\\.(?<Index>\\d+))$|^(?:P(?<Group>\\w)\\.(?<Index>\\d+))/|(?:P(?<Group>\\d)\\[(?<Index>\\d+)\\])|(?:RP?(?<Group>[A-Z])(?<Index>\\d+))|^(?:G?PI?O?(?<Group>[A-Z0-9])_?(?<Index>\\d+))|(?:(?<Group>D?IO)_(?<Index>\\d+))|(?:PT(?<Group>[A-Z])(?<Index>\\d+))|^(?<Group>[a-zA-Z]{1,3})(?<Index>\\d+)$", PinPosition.From(PinSide.Right, PinAlignment.Middle));
